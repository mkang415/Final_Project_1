...
<nav aria-label="Page navigation example">
  <ul class="pagination justify-content-center">
  	...
  	<c:if test="${freePaging.curPage ne 1 }">
  		<li class="page-item"><a class="page-link" href="/board/freelist?search=${freePaging.search}"><span>처음</span></a></li>
  		<li class="page-item"><a class="page-link" href="/board/freelist?curPage=${freePaging.curPage-1 }&search=${freePaging.search}"><span>&laquo;</span></a>
  	</c:if>
	...
	<c:forEach begin="${freePaging.startPage }" end="${freePaging.endPage }" var="i">
		...
		<c:if test="${freePaging.curPage ne i}">
			<li class="page-item">
				<a class="page-link" href="/board/freelist?curPage=${i }&search=${freePaging.search}">${i }</a>
			</li>
		</c:if>
	</c:forEach>
    ...
    <c:if test="${freePaging.curPage ne freePaging.totalPage }">
    	<li class="page-item"><a class="page-link" href="/board/freelist?curPage=${freePaging.curPage+1 }&search=${freePaging.search}"><span>&raquo;</span></a>
    </c:if>
  </ul>
</nav>
